...
int compute_blk_size(int dev_size)
{
	...
	while (1)
	{
		inodes_per_block = block_size / sizeof(struct emxfs_inode);
	...
	return block_size;
#else // Currently, PAGE_SIZE is the only kernel supported fs block size
	return PAGE_SIZE;
#endif
}

void compute_emxfs_sb(int dev_size, int block_size, struct emxfs_sb *sb)
{
	int block_cnt;
	int avg_file_cnt, avg_meta_block_cnt;
	...
	sb->fs_type = EMXFS_SUPER_MAGIC;
	sb->block_size = block_size;
	sb->block_cnt = block_cnt;
	sb->inodes_per_block = block_size / sizeof(struct emxfs_inode);
...
void compute_emxfs_empty_inode(struct emxfs_inode *inode)
{
	inode->file_name[0] = 0;
}

void compute_emxfs_empty_dir_inode(char *dir_name, struct emxfs_inode *inode)
{
	int i;

	strncpy(inode->file_name, dir_name, EMXFS_MAX_FILE_NAME_SIZE);
	inode->file_size = emxfs_dir;
...
void compute_emxfs_empty_file_inode(char *file_name, struct emxfs_inode *inode)
{
	int i;

	strncpy(inode->file_name, file_name, EMXFS_MAX_FILE_NAME_SIZE);
	inode->file_size = 0;
	for (i = 0; i < EMXFS_MAX_ENTRIES_PER_DIR; i++)
	{
...
int make_emxfs(int fd, int dev_size, int block_size)
{
	struct emxfs_sb sb;
	struct emxfs_inode empty_inode, root_inode;
#ifdef ADD_ONE_FILE
	struct emxfs_inode file_inode;
	...
	compute_emxfs_sb(dev_size, block_size, &sb);
	compute_emxfs_empty_inode(&empty_inode);
	compute_emxfs_empty_dir_inode("/", &root_inode);
#ifdef ADD_ONE_FILE
	root_inode.u.dir_entries[0] = sb.root_inode + 1;
	compute_emxfs_empty_file_inode("moksha.txt", &file_inode);
	...
	lseek(fd,
		(sb.inodes_block_start + (sb.root_inode / sb.inodes_per_block)) *
			block_size +
		(sb.root_inode % sb.inodes_per_block) * sizeof(struct emxfs_inode),
		SEEK_SET);
...
int main(int argc, char *argv[])
{
	...
	if (make_emxfs(fd, dev_size, block_size) < 0)
	{
		close(fd);
		return 4;
	}
	printf("emx fs created on device %s [%d bytes = %d blocks] (1 block = %d bytes)\n",
