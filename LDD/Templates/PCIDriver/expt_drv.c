...
static void display_pci_config_space(struct pci_dev *dev)
{
	...
	for (i = 0; i < 6; i++)
	{
		printk(KERN_INFO "Bar %d: 0x%08X-%08X : %08lX : %s %s\n", i,
			pci_resource_start(dev, i), pci_resource_end(dev, i),
...
static int expt_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	//Obtain the Virtual address from PA
